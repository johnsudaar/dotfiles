...
cpu_temp=$(sensors -j 2>/dev/null | jq -r '."thinkpad-isa-0000"."CPU"."temp1_input"' | cut -d'.' -f1)
...
echo "🎧 $song_status $media_artist - $media_song | ⌨ $language | $network_active $network ($ping ms) | 🏋 $loadavg_5min ($cpu_temp°C) | $audio_active $audio_volume% Out:$active_sink_name In:$active_source_name | $battery_pluggedin $battery_charge | $date_and_week 🕘 $current_time"
